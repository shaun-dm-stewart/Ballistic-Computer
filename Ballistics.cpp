...
double Ballistics::calcFR(double Temperature, double Pressure, double RelativeHumidity) {
...
double Ballistics::calcFP(double Pressure) {
...
double Ballistics::calcFT(double Temperature, double Altitude) {
	double Tstd = -0.0036 * Altitude + 59;
	double FT = (Temperature - Tstd) / (459.6 + Tstd);
	return FT;
}

double Ballistics::calcFA(double Altitude) {
	double fa = 0;
	fa = -4e-15 * pow(Altitude, 3) + 4e-10 * pow(Altitude, 2) - 3e-5 * Altitude + 1;
	return (1 / fa);
}

double Ballistics::AtmCorrect(double DragCoefficient, double Altitude, double Barometer, double Temperature, double RelativeHumidity) {
...
double Ballistics::retard(int dragFunction, double dragCoefficient, double Velocity) {
...
int Ballistics::SolveAll(int DragFunc, double DragCoefficient, double Vi, double SightHeight, \
	...
	for (t = 0;; t = t + dt) {
		...
		if (dd >= n && dd > (range - 1)) {
			...
			Solution->Range = (x / 3);														// range in yards
			Solution->Path = currentY * 12;													// path in inches
			Solution->Correction = -RadtoMOA(atan(currentY / currentX));					// correction in MOA
			Solution->TimeInSecs = t + dt;													// time in seconds
			Solution->Windage = Windage(crosswind, Vi, currentX, t + dt) + spinDrift;		// windage in inches
			Solution->WindageMOA = RadtoMOA(atan(Solution->Windage / (12 * currentX)));		// windage in MOA
			Solution->VelocityCombined = v;													// combined velocity
			Solution->VelocityX = vx;														// velocity x
			Solution->VelocityY = vy;														// velocity y
			Solution->SpinDrift = spinDrift;												// spin drift in inches
...
double Ballistics::StabilityFactor(double BulletLength, double Caliber, double BulletWeight, double BarrelTwist)
{
...
double Ballistics::Windage(double WindSpeed, double Vi, double xx, double t) {
	double Vw = WindSpeed * 17.60; // Convert to inches per second.
	return (Vw * (t - xx / Vi));
}

// Headwind is positive at WindAngle=0
double Ballistics::HeadWind(double WindSpeed, double WindAngle) {
	double Wangle = DegtoRad(WindAngle);
	return (cos(Wangle) * WindSpeed);
}

// Positive is from Shooter's Right to Left (Wind from 90 degree)
double Ballistics::CrossWind(double WindSpeed, double WindAngle) {
	double Wangle = DegtoRad(WindAngle);
	return (sin(Wangle) * WindSpeed);
}

double Ballistics::ZeroAngle(int DragFunc, double DragCoefficient, double Vi, double SightHeight, double ZeroRange, double yIntercept) {
